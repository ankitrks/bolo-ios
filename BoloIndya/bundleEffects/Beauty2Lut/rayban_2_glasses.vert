...
void main()
{
    ...
    gl_Position.x *= 0.5;
    vec2 s = vec2(sign(glfx_PROJ[0][0]),sign(glfx_PROJ[1][1]));
    vec2 offsets = vec2(-0.25, 0.0);
    gl_Position.xy -= offsets*s*2.*gl_Position.w;
    var_uv = attrib_uv;
...
#else
    var_bg_uv = s*(gl_Position.xy/gl_Position.w)*0.5 + 0.5;
#endif
