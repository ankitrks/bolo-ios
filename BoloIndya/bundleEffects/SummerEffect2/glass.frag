...
vec3 shade(vec3 lightPos, vec3 Kd, vec3 P, vec3 N)
{
    ...
    vec3 R = reflect(-L, N);
    vec3 V = normalize(vec3(var_js_resolution.x*0.5, var_js_resolution.y*0.5, var_cam_pos.y) - P);
    ...
    float fr = fresnel(V, N, 0.3);
    
    #ifdef HAS_REFLECTION
    	vec3 Ir = fr * texture(tex_skybox, reflect(-V, N)).rgb;
    	I = I + Ir;
    #endif
    
    #ifdef HAS_REFRACTION
    	vec3 It = (1.0-fr) * texture(tex_skybox, refract(-V, N, 0.8)).rgb;
    	I = I + It;
    #endif
...
#endif
    vec3 reflect_color = shade(var_cam_pos, vec3(0.0), var_v, var_n);
    vec3 org_color = l2g(color);
    frag_color = vec4(reflect_color + org_color,opacity);
