...
vec4 sharpen(vec4 originalColor, float factor) {
    vec4 total = 5.0 * originalColor - texture(glfx_BACKGROUND, sp0.zw) - texture(glfx_BACKGROUND, sp1.zw) - texture(glfx_BACKGROUND, sp2.zw) - texture(glfx_BACKGROUND, sp3.zw);
...
vec4 softSkin(vec4 originalColor, float factor) {
    ...
    mat4 nextColor;
    nextColor[0] = texture(glfx_BACKGROUND, sp0.xy);
    nextColor[1] = texture(glfx_BACKGROUND, sp1.xy);
    nextColor[2] = texture(glfx_BACKGROUND, sp2.xy);
    nextColor[3] = texture(glfx_BACKGROUND, sp3.xy);
