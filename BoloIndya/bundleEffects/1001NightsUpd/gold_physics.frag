...
{
    return vec3(blend_overlay(a.r, b.r), blend_overlay(a.g, b.g), blend_overlay(a.b, b.b));
}
...
void main()
{
	...
	float relfectionSpecular = pow (max (0.0, dot (reflect (forward, normal.xyz), lightDirection)), SPECULAR_POWER) * SPECULAR_INTENSITY;
     ...
     vec4 reflect_color;
	reflect_color.rgb = (AMBIENT + relfectionDiffuse) * COLOR + relfectionSpecular;
    
	reflect_color.r = raycast (origin, forward, normal, reflect_color.r, vec3 (1.0, 0.0, 0.0));
	reflect_color.g = raycast (origin, forward, normal, reflect_color.g, vec3 (0.0, 1.0, 0.0));
	reflect_color.b = raycast (origin, forward, normal, reflect_color.b, vec3 (0.0, 0.0, 1.0));

	reflect_color.a = 1.0;

    frag_color = vec4(blend_overlay(color, reflect_color.rgb), opacity);
